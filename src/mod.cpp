...
// dusk::resetForSpeedrunMode is hooked to prevent speedrun mode from overriding wall-clock
// sync while this mod is enabled: the mod takes precedence over speedrun mode.
...
// PRE hook: when the mod is enabled, skip dusk::resetForSpeedrunMode so that activating
// speedrun mode cannot override the wall-clock time sync.  The mod takes precedence; the
// speedrun mode option is effectively disabled while this mod is on.
static HookAction on_reset_for_speedrun_pre(ModContext*, void*, void*, void*) {
    if (is_mod_enabled()) {
        return HOOK_SKIP_ORIGINAL;
    }
    return HOOK_CONTINUE;
...
MOD_EXPORT ModResult mod_initialize(ModError*) {
    ...
    result = mods::hook_add_pre<ResetForSpeedrunMode>(svc_hook, on_reset_for_speedrun_pre);
    if (result != MOD_OK) {
        // Non-fatal: the symbol may be absent on some Dusklight builds. Log a warning so
        // the user knows why speedrun mode is not blocked while this mod is enabled.
        svc_log->warn(mod_ctx, "failed to install on_reset_for_speedrun_pre; speedrun mode will not be blocked while mod is enabled");
    }
